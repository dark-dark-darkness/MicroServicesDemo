...
namespace MicroServicesDemo.PlatformService.Modules;
...
public class PlatformsModule : ICarterModule
{
    ...
    private static async Task<Results<Created<PlatformReadDto>, BadRequest>>
            Create([FromBody] PlatformCreateDto dto, [FromServices] IPlatformRepository repository)
    {
        var e = dto.Adapt<Platform>();
        await repository.AddAsync(e);
        await repository.SaveChangesAsync();
        return TypedResults.Created($"/api/platforms/{e.Id}", e.Adapt<PlatformReadDto>());
    }
